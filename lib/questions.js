...
export function getNewTextComponent(demoId, textComponentProps, newProps) {
    switch (demoId) {
        ...
        case demoIds.VOICE_ALPHABET_CZ_1: {
            ...
            // Get cached audio element for better performance and Safari compatibility
            const cachedAudio = audioCache.getCachedAudio(textComponentProps.audioUrl);
            if (cachedAudio) {
                cachedAudio.controls = true;
                cachedAudio.autoPlay = true;
                return cachedAudio;
            }
    ...
    if (demoId) {
        switch (demoId) {
            ...
            case demoIds.VOICE_ALPHABET_CZ_1: {
                ...
                // Get cached audio element for better performance and Safari compatibility
                const cachedAudio = audioCache.getCachedAudio(audioUrl);
                if (cachedAudio) {
                    cachedAudio.controls = true;
                    cachedAudio.autoPlay = true;
                    textComponent = <div>{cachedAudio}</div>;
                } else {
                    // Fallback to regular audio element if not cached
                    textComponent = <div>
...
export const isIconDisplay = (demoId) => {
    const iconDisplays = [demoIds.DICE_ADD_1, demoIds.DICE_LARGER_1, demoIds.LARGER_1, demoIds.LARGER_2, demoIds.LARGER_3, demoIds.NEXT_NUMBER_1, demoIds.WORDS_1, demoIds.CHESS_1, demoIds.PATTERNS_1];
