 ...
 */
export const getResult = (question, optionValue) => {
    let ok = false;
    let text = 'Nehodnoceno';
    let correctAnswer = null;

    if (optionValue) {
        correctAnswer = question.steps?.[question.currentStep]?.solution;
        if (question.steps?.[question.currentStep]?.solution === optionValue) {
            ok = true;
        }
    } else {
...
export const isIconDisplay = (demoId) => {
    const iconDisplays = ['dice-add-1', 'dice-larger-1', 'larger-1', 'words-1'];
...
export const isSingleStep = (demoId) => {
    const multiSteps = ['words-1'];
    return !multiSteps.includes(demoId);
};

export const doEmojis = ({ evt, result /*, demoId */, emojiCount }) => {
